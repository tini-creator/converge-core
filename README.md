...
Full free5GC control plane (AMF, AUSF, CHF, NRF, NSSF, PCF, SMF, UDM, UDR, NEF) + UPF + WebUI, deployed via Docker Compose, with a UERANSIM gNB/UE for RAN simulation. WebConsole is accessed via SSH tunnel only — deliberately not exposed on the public interface.
...
**Important:** the UPF requires the `gtp5g` kernel module at the version specified in the free5gc-compose README for your checked-out tag (currently `v0.9.5` — this has changed between releases, so always check rather than assume):
...
Register a test subscriber via the WebConsole (`http://localhost:5000` if tunneled over SSH — recommended over exposing it publicly) before running UE registration.
